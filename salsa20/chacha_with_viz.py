...
def diagonalround(y: list[Word], doubleround_count: int) -> list[Word]:
    ...
    visualizer.qround(y, [0, 5, 10, 15], doubleround_count, 1, False)
    visualizer.qround(y, [1, 6, 11, 12], doubleround_count, 2, False)
    visualizer.qround(y, [2, 7, 8, 13], doubleround_count, 3, False)
    visualizer.qround(y, [3, 4, 9, 14], doubleround_count, 4, False)

    return z


def columnround(x: list[Word], doubleround_count: int) -> list[Word]:
    ...
    visualizer.qround(y, [0, 4, 8, 12], doubleround_count, 1, True)
    visualizer.qround(y, [1, 5, 9, 13], doubleround_count, 2, True)
    visualizer.qround(y, [2, 6, 10, 14], doubleround_count, 3, True)
    visualizer.qround(y, [3, 7, 11, 15], doubleround_count, 4, True)

    return y


def doubleround(x: list[Word], doubleround_count: int) -> list[Word]:
    ...
    if len(x) != 16:
        raise ValueError('x must be a 16-word list')
    # visualizer.block(x)
    y = diagonalround(columnround(x, doubleround_count), doubleround_count)
    # visualizer.block(y)
    return y
...
def encrypt(key: bytes, nonce: bytes, msg: bytes, viz_speed: float, iv: int = 0) -> bytes:
    ...
    global visualizer
    visualizer = ChaChaViz(viz_speed)
